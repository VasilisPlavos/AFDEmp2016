...
public class TestingBootCamp {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < admin.size(); ++i){
			System.out.println(admin.getAnArrayListOfBootCamps().get(i));			
		}		
		
		//Add yourself as a Bootcamp candidate name, surname, and age
		BootCampCandidates me = new BootCampCandidates("Vasilis", "Plavos", 25);
		admin.addBootCampCandidate(me);
		
		
		//Remove from the list member Marios if exist print message "success" if not "failed to find"
		admin.removeBootCampCandidate("Marios");		
		...
		System.out.println(admin.searchMember("Louis").toString());
